...
>(({ className, ...props }, ref) => (
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        "pointer-events-none flex items-center justify-center h-5 w-5 rounded-full bg-background shadow-lg ring-0 transition-all duration-300 data-[state=checked]:translate-x-6 data-[state=unchecked]:translate-x-0"
      )}
    >
      {props.checked ? 
        <MoonIcon className="h-3 w-3 text-primary" /> : 
        <SunIcon className="h-3 w-3 text-amber-400" />
      }
    </SwitchPrimitives.Thumb>
  </SwitchPrimitives.Root>
))
